class CPU:
	...
	def __init__(self, addr_trans):
		self.PC = 0
		self.IR = 0
		self.Register = [0] * 32
		self.ALUOuter = 0
		self.IsBusy = {'PC': False, 'IR': False, 'Register': False, 'ALU': False}
		self.address_translator = addr_trans
	...
	def IF(self):
		...
		memory = self.address_translator(self.PC)
		# 从内存中取出指令，程序寄存器指向下一条指令位置
		self.IR = memory.read(self.PC, 4)
		self.PC = self.PC + 4
	...
	def MEM(self):
		...
		# cyh添加
		if self.opcode == 0x00:
			self.ALUOuter = memory.read(self.ALUOuter, 4)
		elif self.opcode == 0x01:
